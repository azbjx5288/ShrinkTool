...
package com.shrinktool.base.thread;
...
public class ThreadPool {
    ...
    public <T> Future<T> submit(Job<T> job, FutureListener<T> listener, boolean returnInMain) {
        Worker<T> w = new Worker<>(job, listener, returnInMain, returnInMain ? PRIORITY_FLAG_MUTUAL : 0);
        mExecutor.execute(w);
        return w;
    }
    
    public <T> Future<T> submit(Job<T> job, FutureListener<T> listener, boolean returnInMain, int priorityFlag) {
        Worker<T> w = new Worker<>(job, listener, returnInMain, priorityFlag);
        mExecutor.execute(w);
        return w;
    }
